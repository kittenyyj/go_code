...
func main() {
	...
	fmt.Printf("cat1的地址=%p\n", &cat1.Age)
	fmt.Printf("cat1的地址=%p\n", &cat1.Color)
	fmt.Printf("cat1的地址=%p  %d\n", &cat1.Scores,unsafe.Sizeof(cat1.Scores))
	fmt.Printf("cat1的地址=%p  %d\n", &cat1.Pep,unsafe.Sizeof(cat1.Pep))
	fmt.Printf("cat1的地址=%p\n", &cat1.Hobby)
	cat1.Name = "小白"
	cat1.Age = 3
	cat1.Color = "白色"
	cat1.Hobby = "吃<・)))><<"
	cat1.Scores = append(cat1.Scores, 100)
	cat1.Pep = make(map[string]string)
	cat1.Pep["xm"] = "yyj"
